<template>
  <div>
    <page-link only-container :to="{name: 'private.orders.details', params: { id: order._id }}">
      <template v-slot:default="{href, navigate}">
        <ion-item button detail :href="href" @click="navigate">
          ...
          <div slot="end" class="d-flex ion-align-items-center">
            <ion-chip color="danger" class="unreadCounter small" mode="ios" v-if="order.unreadCount">{{
                order.unreadCount
              }}
            </ion-chip>
